...
TEMPLATEFILE="Transfer_08-09-20_SC_diffApt_d"$PARAMETER".m"
...
FOLDER="SC_Prot_diffApt_d"$PARAMETER
...
writer_Function(){
    ...
    then
		...
		echo "echo Finished: $LOCALFILEDBACK" >> $SCRIPTFILE
	elif [[ $option == "H" ]] 
    then
		...
		echo "echo Finished: $LOCALFILEDBACK" >> $SCRIPTFILEH
    elif [[ $option == "G" ]] 
    then
		...
		echo "echo Finished: $LOCALFILEDBACK" >> $SCRIPTFILEG
    elif [[ $option == "C" ]] 
    then
    ...
    sed -i "s+^Export.*$+Export[\"MergerTransport/SC_Prot_diffApt_d$PARAMETER/a\"<>ToString[IntegerPart[a*10000]]<>\"/TransferResult_08-09-20_SC_diffApt_$option\_d$PARAMETER\_a\"<>ToString[IntegerPart[a*10000]]<>\"_\"<>ToString[Bins[[1]]]<>\"-\"<>ToString[Bins[[2]]]<>\".txt\",BinwYShiftPrec44OriginalBinsb0NewBGrad,\"Table\"];+" $LOCALFILE
